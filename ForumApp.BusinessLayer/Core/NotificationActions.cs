...
namespace ForumApp.BusinessLayer.Core
{
    public class NotificationActions
    {
        ...
        public NotificationActions(ForumDbContext context, IHubNotifierAction hubNotifier)
        {
            _context = context;
            _hubNotifier = hubNotifier;
        }

        internal async Task<IReadOnlyList<NotificationResponseDto>> GetUserNotificationsExecution(int userId, CancellationToken ct = default)
        {
        ...
        internal async Task<int> GetUnreadCountExecution(int userId, CancellationToken ct = default)
        {
            return await _context.Notifications
                .CountAsync(n => n.RecipientId == userId && !n.IsRead, ct);
        }

        internal async Task<ActionResponse> MarkAsReadExecution(int notificationId, int userId, CancellationToken ct = default)
        {
        ...
        internal async Task<ActionResponse> MarkAllAsReadExecution(int userId, CancellationToken ct = default)
        {
        ...
        internal async Task<ActionResponse> DeleteNotificationExecution(int notificationId, int userId, CancellationToken ct = default)
        {
            var notification = await _context.Notifications
                .FirstOrDefaultAsync(n => n.Id == notificationId && n.RecipientId == userId, ct);
        ...
        internal async Task CreateAndSendExecution(
            int recipientId,
